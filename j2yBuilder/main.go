...
func main(){
	...
	mustRunCmd([]string{"go","test","-v","github.com/bronze1man/json2yaml/j2yLib"},nil)
	...
	for _,info:=range buildInfoList{
		mustRunCmd([]string{"go","install","-ldflags","-s -w","-gcflags=-trimpath="+gopath,"github.com/bronze1man/json2yaml"},map[string]string{
			"GOOS":info.goos,
			"GOARCH":info.goarch,
		})
		var outputPath string
		if info.goarch == runtime.GOARCH && info.goos == runtime.GOOS{
			outputPath = filepath.Join(gopath,"bin","json2yaml")
		}else{
			outputPath = filepath.Join(gopath,"bin",info.goos+"_"+info.goarch,"json2yaml")
		}
		if info.goos=="windows"{
			outputPath+=".exe"
		}
		filePath:="json2yaml_"+info.goos+"_"+info.goarch
		if info.goos=="windows"{
			filePath+=".exe"
		}
	...
	mustZipDir(filepath.Join(gopath,"tmp","fileZip"),filepath.Join(gopath,"tmp","file","json2yaml_all.zip"))
}
